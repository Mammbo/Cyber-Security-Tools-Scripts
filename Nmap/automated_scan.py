...
def main():
    ...
    scan1 = icmp_echo_host_scan(target, os.getcwd(), formatted_time)
    scan2 = icmp_netmask_host_scan(target, os.getcwd(), formatted_time)
    scan3 = icmp_timestamp_host_scan(target, os.getcwd(), formatted_time)
    scan4 = tcp_syn_host_scan(target, os.getcwd(), formatted_time)


    hosts = parseDiscoverXml(scan1)
    hosts += parseDiscoverXml(scan2)
    hosts += parseDiscoverXml(scan3)
    hosts += parseDiscoverXml(scan4)

    target = convertToNmapTarget(hosts)
    #this function is here so when the 1000 port scan is executed its output can be used for the parse the discovered ports function
    location_of_port_scan = tcpsyn_port_scan_1000(target, os.getcwd(), formatted_time)

    targets = parseDiscoverPorts(location_of_port_scan)
    for target in targets:
        element = target.split()
        target_ip = element[0]
        target_ports = element[1]
   
   #stage 2 scans after we get the important information to conduct these scans 
